...
\[
\rho: B_n\to\TL_n^\times,\qquad \rho(\sigma_i)=e_i-vI,
\] 
...
The Jones representations of $B_n$ are useful when paired with a certain trace on $\TL_n$. 
The Markov trace on $\TL_n$ is defined on basis elements by closing the element's diagram 
and scaling by $\delta$ for each closed circle. 
Then the (unnormalised) Jones polynomial of a braid's closure is recovered from the
Markov trace of the braid's image in $\TL_n$, up to a factor determined by the braid's writhe. 
That is, we have
\[
J_{\cl(\beta)}(v)=(-v^2)^{-w(\beta)}\,\trm\bigl(\rho(\beta)\bigr).
\]

Note \(\trm\) is a trace map on $\TL_n$, so on the matrix algebra side, it corresponds to a linear combination 
of plain matrix traces. That is, there are scalars \(c_\lambda\) such that
\[
J_{\cl(\beta)}(v)
 =(-v^2)^{-w(\beta)}\,\trm(\rho(\beta))
 =(-v^2)^{-w(\beta)}\sum_{\substack{\lambda \vdash n \\ \ell(\lambda)\le 2}}  
c_\lambda\,\Tr(\rho_\lambda(\beta)).
\] 
Writing \(\lambda=(n-r,r)\), we can prove the formula $c_\lambda=\qq{n-2r+1}$, where $\qq{m}$ is quantum $m$. 
...
\subsubsection*{\texttt{tl\_jones.py}}
\begin{enumerate}
...
\subsubsection*{\texttt{test\_tl\_jones.py}}
\begin{enumerate}
  ...
  \item \texttt{test\_05a\_braid\_generator\_formula}
...
It uses the formula $\rho(\sigma_i) = e_i - vI$.
...
Computes the unnormalised Jones polynomial of the closure of a braid word using the formula
$J_{\mathrm{cl}(\beta)}(v)
= (-v^2)^{-w(\beta)}
...
Checks Temperley--Lieb algebra relations for the matrices of the \(e_i\):
\[
...
\paragraph{\texttt{test\_05a\_braid\_generator\_formula}}
Checks that the public braid-generator matrix is the matrix of \(e_i - vI\) on a
small representative range of \((n,r,i)\).
